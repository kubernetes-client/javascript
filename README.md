...
`request` was migrated to `node-fetch` as the HTTP(S) backend for release `1.0.0` tracked in [#754](https://github.com/kubernetes-client/javascript/issues/754)

`node-fetch` was migrated to `undici` which is the native node.js fetch package for release `2.0.0` tracked in [#2306](https://github.com/kubernetes-client/javascript/issues/2306)
