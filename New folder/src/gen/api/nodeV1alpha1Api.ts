...
export class NodeV1alpha1Api {
     ...
     * @param pretty If \&#39;true\&#39;, then the output is pretty printed.
     */
    public async readRuntimeClass (name: string, pretty?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: V1alpha1RuntimeClass;  }> {
        const localVarPath = this.basePath + '/apis/node.k8s.io/v1alpha1/runtimeclasses/{name}'
            .replace('{' + 'name' + '}', encodeURIComponent(String(name)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
