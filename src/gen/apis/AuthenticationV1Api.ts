...
export class AuthenticationV1ApiResponseProcessor {
     ...
     */
     public async createSelfSubjectReview(response: ResponseContext): Promise<V1SelfSubjectReview > {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async createTokenReview(response: ResponseContext): Promise<V1TokenReview > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
