...
export class AutoscalingV2beta1Api {
     ...
     * @param pretty If \&#39;true\&#39;, then the output is pretty printed.
     */
    public async readNamespacedHorizontalPodAutoscaler (name: string, namespace: string, pretty?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: V2beta1HorizontalPodAutoscaler;  }> {
        const localVarPath = this.basePath + '/apis/autoscaling/v2beta1/namespaces/{namespace}/horizontalpodautoscalers/{name}'
            .replace('{' + 'name' + '}', encodeURIComponent(String(name)))
            .replace('{' + 'namespace' + '}', encodeURIComponent(String(namespace)));
        let localVarQueryParameters: any = {};
