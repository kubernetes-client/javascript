...
async function stepRead(record) {
    ...
    try {
        ...
        log_1.log.error(`Cannot read file, ${error.code}`, record.file);
...
class Walker {
    ...
    appendFileInFolder(task) {
        ...
        const realFile = common_1.toNormalizedRealPath(task.file);
    ...
    async stepDerivatives_ALIAS_AS_RELATIVE(record, marker, derivative) {
        const file = common_1.normalizePath(path_1.default.join(path_1.default.dirname(record.file), derivative.alias));
        let stat;
        try {
            ...
            const level = debug ? 'debug' : 'warn';
            log_1.log[level](`Cannot stat, ${error.code}`, [
                file,
    ...
    async stepDerivatives_ALIAS_AS_RESOLVABLE(record, marker, derivative) {
        ...
        try {
            newFile = await follow_1.follow(derivative.alias, {
                basedir,
        ...
        for (const newPackage of newPackages) {
            let newFile2;
            try {
                newFile2 = await follow_1.follow(derivative.alias, {
    ...
    async step_STORE_STAT(record) {
        if (record[common_1.STORE_STAT])
            return;
        const realPath = common_1.toNormalizedRealPath(record.file);
        ...
        try {
            ...
            log_1.log.error(`Cannot stat, ${error.code}`, record.file);
            throw log_1.wasReported(error);
        }
    ...
    async readDictionary(marker) {
        ...
        const pkgConfig = (_a = marker.config) === null || _a === void 0 ? void 0 : _a.pkg;
        if (pkgConfig) {
