...
const { version } = JSON.parse(fs_extra_1.readFileSync(path_1.default.join(__dirname, '../package.json'), 'utf-8'));
...
async function needWithDryRun({ forceBuild, nodeRange, platform, arch, }) {
    const result = await pkg_fetch_1.need({
        dryRun: true,
        forceBuild,
        nodeRange,
        platform,
...
async function exec(argv2) {
    ...
    const argv = minimist_1.default(argv2, {
        boolean: [
            'b',
            'build',
            'bytecode',
    ...
    if ((await fs_extra_1.stat(input)).isDirectory()) {
    ...
    if (isConfiguration(input)) {
        inputJson = JSON.parse(await fs_extra_1.readFile(input, 'utf-8'));
    ...
    for (const target of targets) {
        ...
        if (f && bytecode) {
            ...
            if (f.platform === 'macos') {
                ...
                await fs_extra_1.remove(signedBinaryPath);
                fs_extra_1.copyFileSync(f.binaryPath, signedBinaryPath);
                try {
                    mach_o_1.signMachOExecutable(signedBinaryPath);
                }
                catch (_b) {
                    throw log_1.wasReported('Cannot generate bytecode', [
            ...
            if (f.platform !== 'win') {
                await chmod_1.plusx(f.binaryPath);
            }
        }
    }
    // marker
    ...
    const walkResult = await walker_1.default(marker, entrypoint, addition, params);
    entrypoint = walkResult.entrypoint;
    records = walkResult.records;
    symLinks = walkResult.symLinks;
    const refineResult = refiner_1.default(records, entrypoint, symLinks);
    entrypoint = refineResult.entrypoint;
    records = refineResult.records;
    symLinks = refineResult.symLinks;
    const backpack = packer_1.default({ records, entrypoint, bytecode, symLinks });
    log_1.log.debug('Targets:', JSON.stringify(targets, null, 2));
    for (const target of targets) {
        if (target.output && fs_extra_1.existsSync(target.output)) {
            if ((await fs_extra_1.stat(target.output)).isFile()) {
                await fs_extra_1.remove(target.output);
            }
            else {
                throw log_1.wasReported('Refusing to overwrite non-file output', [
                    target.output,
                ]);
            }
        }
        else if (target.output) {
            await fs_extra_1.mkdirp(path_1.default.dirname(target.output));
        }
        await producer_1.default({
            backpack,
            bakes,
        ...
        if (target.platform !== 'win' && target.output) {
            if (target.platform === 'macos') {
                ...
                const buf = mach_o_1.patchMachOExecutable(fs_extra_1.readFileSync(target.output));
                ...
                try {
                    ...
                    mach_o_1.signMachOExecutable(target.output);
                }
                catch (_c) {
            ...
            await chmod_1.plusx(target.output);
        }
    }
