...
function nativePrebuildInstall(target, nodeFile) {
    ...
    // run prebuild
    child_process_1.execFileSync(prebuildInstall, [
        '--target',
        nodeVersion,
        '--platform',
...
function producer({ backpack, bakes, slash, target, symLinks, doCompress, }) {
    return new Promise((resolve, reject) => {
        ...
        for (const [key, value] of Object.entries(symLinks)) {
            const k = common_1.snapshotify(key, slash);
            const v = common_1.snapshotify(value, slash);
                ...
                if (stripes.length) {
                    ...
                    if (stripe.buffer) {
                        if (stripe.store === common_1.STORE_BLOB) {
                            const snap = common_1.snapshotify(stripe.snap, slash);
                            return fabricator_1.fabricateTwice(bakes, target.fabricator, snap, stripe.buffer, (error, buffer) => {
                                if (error) {
                        ...
                        return cb(null, pipeMayCompressToNewMeter(into_stream_1.default(stripe.buffer)));
                    }
                    if (stripe.file) {
                        if (stripe.file === target.output) {
                            return cb(log_1.wasReported('Trying to take executable into executable', stripe.file), null);
                        }
                        assert_1.default.strictEqual(stripe.store, common_1.STORE_CONTENT); // others must be buffers from walker
                        if (common_1.isDotNODE(stripe.file)) {
                            try {
                                ...
                                log_1.log.debug(`prebuild-install failed[${stripe.file}]:`, err);
                            }
                        }
